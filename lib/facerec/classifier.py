...
class NearestNeighbor(AbstractClassifier):
    ...
    def predict(self, q):
        ...
        sorted_y_and_convert = sorted_y.astype(np.int32)
        hist = dict((key,val) for key, val in enumerate(np.bincount(sorted_y_and_convert)) if val)
        ...
        # item
        print "distance : " + str(sorted_distances[0])
